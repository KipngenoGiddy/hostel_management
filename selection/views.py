...
def warden_profile(request):
    ...
    if user is not None:
        ...
        return HttpResponse('Login to proceed')
...
def empty_rooms(request):
    if request.user.is_authenticated and request.user.is_guest:
        empty_rooms = Room.objects.filter(student=None).order_by('no')
        context = {'rooms': empty_rooms}
...
def book_room(request, room_id):
    if request.user.is_authenticated and request.user.is_student:
        try:
            ...
            if room.student:
                return HttpResponse('Room is already occupied')
            else:
                student = request.user.student
                student.room = room
                student.save()
...
@login_required
def unbook_room(request):
    if request.user.is_authenticated and request.user.is_guest:
        try:
            student = request.user.guest
            if student.room:
                student.room = None
                student.save()
                messages.success(request, 'Room unbooking successful!')
...
def guest_profile(request):
    try:
        ...
        raise Http404("Guest profile does not exist")